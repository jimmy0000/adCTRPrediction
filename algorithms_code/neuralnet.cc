...
const double REGULARIZATIONCONSTANT = 0.01;
...
const string TRAINING_FILE = "F:\\track2\\training_interactionfeatures3.txt";
const string VALIDATION_FILE = "F:\\track2\\validation_interactionfeatures3.txt";
const string SMALLVALIDATION_FILE = "F:\\track2\\smallvalidation_interactionfeatures3.txt";
const string TESTING_FILE = "F:\\track2\\testing_interactionfeatures3.txt";
const string SUBMISSION_FILE = "F:\\track2\\submission_interactionfeatures3.txt";
const string OUTPUT_TRAINING_FILE = "F:\\track2\\output_training_neural_net.txt";
const string OUTPUT_VALIDATION_FILE = "F:\\track2\\output_validation_neural_net.txt";
const string OUTPUT_TESTING_FILE = "F:\\track2\\output_testing_neural_net.txt";
const string OUTPUT_SUBMISSION_FILE = "F:\\track2\\output_submission_neural_net.txt";
const string OUTPUT_WMAE_FILE = "F:\\track2\\output_wmae_neural_net.txt";
const string ADTITLE_TOKENS_FILE = "C:\\Users\\Arvind\\Desktop\\data\\titleid_tokensid3.txt";
const string ADDESCRIPTION_TOKENS_FILE = "C:\\Users\\Arvind\\Desktop\\data\\descriptionid_tokensid3.txt";
const string KEYWORD_TOKENS_FILE = "C:\\Users\\Arvind\\Desktop\\data\\purchasedkeywordid_tokensid3.txt";
const string DISPLAYURL_PROPERTIES_FILE = "C:\\Users\\Arvind\\Desktop\\data\\displayurl_clicks_impressions_uniqueusers3.txt";
const string ADVERTISER_PROPERTIES_FILE = "C:\\Users\\Arvind\\Desktop\\data\\advertiser_properties3.txt";
const string USER_FEATURES_FILE = "C:\\Users\\Arvind\\Desktop\\data\\userid_profile3.txt";
const string TOKENCOUNTS_UNIQUE_QUERIES_FILE = "C:\\Users\\Arvind\\Desktop\\data\\analysis_queryid_tokensid3.txt";
const string TOKENCOUNTS_TRAINING_QUERIES_FILE = "C:\\Users\\Arvind\\Desktop\\data\\analysis_training_querytokensid3.txt";
const string TOKENCOUNTS_TRAINING_KEYWORDS_FILE = "C:\\Users\\Arvind\\Desktop\\data\\analysis_training_keywordtokensid3.txt";
const string TOKENCOUNTS_ADTITLE_FILE = "C:\\Users\\Arvind\\Desktop\\data\\analysis_titleid_tokensid3.txt";
const string TOKENCOUNTS_ADDESCRIPTION_FILE = "C:\\Users\\Arvind\\Desktop\\data\\analysis_descriptionid_tokensid3.txt";
const string TOKENCOUNTS_KEYWORD_FILE = "C:\\Users\\Arvind\\Desktop\\data\\analysis_purchasedkeywordid_tokensid3.txt";
...
void RunNeuralNet(string file, int phase, int curiteration)
{
  ...
  {
    cout<<"RunNeuralNet function is invoked only during the training, validation and testing phases"<<endl;
  ...
  while(iteration < MAXITERATIONS)
  {
    ...
    while(!fin.eof())
    {
      ...
      //NOTE: use sigmoid function here, because we want the click through rate to lie in [0, 1]
      layer = numlayers - 1;
      numberOfFromNodes = numhiddennodes;
      output[layer][0] = bias[layer][0];
      REP(fromNode, numberOfFromNodes)output[layer][0] += weight[layer][fromNode][0] * output[layer - 1][fromNode];
      output[layer][0] = SAFESIGMOID(output[layer][0]);
      
      double yhat = output[numlayers - 1][0];
      ...
      {        
        ...
        //NOTE: We had used sigmoid as the activation function for the output layer
        double sigmoidval = SAFESIGMOID(yhat);
        layer = numlayers - 1;
        double outputlayererror = sigmoidval * ( 1 - sigmoidval ) * impressions;
        if(yhat < target - target * EPSILON)outputlayererror *= 1;
        else if(yhat > target + target * EPSILON)outputlayererror *= -1;
        ...
        layererror[layer][0] = outputlayererror;
        ...
        REP(fromNode, numberOfFromNodes)
          REP(toNode, numberOfToNodes)
            weight[layer][fromNode][toNode] = weight[layer][fromNode][toNode] + LEARNINGRATE * (layererror[layer][toNode] * features[fromNode] - REGULARIZATIONCONSTANT * weight[layer][fromNode][toNode]);
        ...
        FOR(layer, 1, numlayers)
        {
          ...
          REP(fromNode, numberOfFromNodes)
            REP(toNode, numberOfToNodes)
              weight[layer][fromNode][toNode] = weight[layer][fromNode][toNode] + LEARNINGRATE * (layererror[layer][toNode] * output[layer - 1][fromNode] - REGULARIZATIONCONSTANT * weight[layer][fromNode][toNode]);
        }
        ...
        REP(layer, numlayers)
        {
          ...
          REP(node, numberOfNodesInLayer)bias[layer][node] = bias[layer][node] + LEARNINGRATE * (layererror[layer][node] - REGULARIZATIONCONSTANT * bias[layer][node]);
        }
      }
      ...
      {
        RunNeuralNet(SMALLVALIDATION_FILE, PHASE_SMALLVALIDATION, iteration);
      }
    }
    ...
    else  //evaluate performance on the validation set and do another pass over the training file
    {
      ...
      RunNeuralNet(TESTING_FILE, PHASE_TESTING, iteration);
...
void OutputSubmission(string inputfile, string outputfile)
{
  ...
  while(!fin.eof())
  {
    ...
    if(iteration < 5)
    {
      dbg(iteration); dbg(impressions); dbg(target); dbg(yhat); dbg(displayurl); dbg(advertiserid); dbg(keywordid); dbg(adtitleid); dbg(addescid); dbge(userid);
      cout<<"Feature Values:";
      REP(i, numfeatures)cout<<" "<<features[i];
      cout<<endl;
